const zhCn = {
  ...
  settings: {
    ...
    account: {
      ...
      antigravity_cli_executable: 'Antigravity CLI 可执行文件',
      antigravity_cli_executable_desc:
        'agy CLI 的独立路径。准入补丁只能作用于此文件，不会修改主程序或 IDE。',
      antigravity_cli_executable_placeholder: '例如：C:\\Program Files\\Antigravity\\agy.exe',
      detect_antigravity_cli: '自动探测',
      agy_cli_detected: '已探测到 Antigravity CLI',
      agy_cli_detect_failed: '未找到 Antigravity CLI',
      agy_patch_desc:
        '备份 CLI，确认 ARM64 或 x86_64 准入模式唯一匹配后写入补丁；macOS Mach-O 会自动重新签名。',
      agy_patch_action: '应用准入补丁',
      agy_patch_success: 'Antigravity CLI 补丁已应用',
      agy_patch_already_applied: '该补丁已经应用',
      agy_patch_failed: '无法修补 Antigravity CLI',
      agy_patch_result: '格式：{{format}}；架构：{{architectures}}；备份：{{backupPath}}',
      agy_patch_no_backup: '无需创建',
  ...
  cloud: {
    ...
    localImport: {
      trigger: '扫描本地账号',
      title: '导入本地账号',
      description: '扫描 Antigravity 的本地登录凭据，核对已验证账号后再确认导入。',
  ...
  proxy: {
    ...
    'open-code': {
      title: 'OpenCode 同步',
      description: '同步托管 Provider，同时保留注释和用户手工维护的 JSONC 格式。',
      synced: '已同步',
      ...
      'backup-notice': '备份会保留注释和格式；凭证会替换为无效占位符，恢复时再注入当前有效 Key。',
      sync: '配置并同步 OpenCode',
      restore: '恢复备份',
      clear: '清除托管配置',
      revoke: '撤销专用 Key',
      'model-dialog-title': '选择 OpenCode 模型',
      'model-dialog-description': '所选模型会被添加或更新；未选择的已有模型不会被删除。',
      ...
      'config-redacted-notice': '在此预览到达渲染进程前，注释会被省略，敏感字段会被脱敏。',
